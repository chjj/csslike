...
var rule = function() {
  ...
  while (next().type !== 'end') { // rule_end
    switch (token.type) {
      case 'rule':
        token.selector = 
          select(selector, token.selector);
        ; // FALL-THROUGH
      case 'nested_at':
        below.push(tok());
        break;
...
var select = function(parent, child) {
  return parent
    .split(/\s*,\s*(?![^\[]*["'])/)
    .map(function(s) {
...
var imports = function(path) {
  ...
  var path = path.replace(/['"]/g, '') 
    ...
    , original = options.dir;

  path = join(options.dir, path);

  options.dir = dirname(path);
  css = parse(lex(css), options);
  options.dir = original;
